...
public class RandomEventSystem
{
	...
	/// <summary>幸运值，整数，单位点（1点幸运值=1权重单位）</summary>
	private int luckValue = 0;

	/// <summary>
	/// 事件触发后的回调委托，参数是触发的事件数据，null表示未触发事件
	/// </summary>
	public event Action<RandomEventsDatabase.RandomEventData> OnEventTriggered;

	public RandomEventSystem(RandomEventsWeightDatabase weightDatabase, RandomEventsDatabase eventsDatabase)
	{
