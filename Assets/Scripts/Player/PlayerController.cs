...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        randomEventManager.SetDatabases(
            GameDataManager.Instance.RandomEventsWeightData,
            GameDataManager.Instance.RandomEventsData,
            GameDataManager.Instance.ItemData
        );
        randomEventManager.SetEventUI(randomEventUI);
        randomEventManager.OnEventTriggered += OnRandomEventTriggered;
        randomEventManager.SetBaseTriggerChance(1f); // 触发概率由PlayerController控制
    }
    ...
    void FixedUpdate()
    {
        ...
        // 体力消耗逻辑
        HandleStaminaConsumption(distanceMoved);

        // 事件触发判定
        HandleEventTrigger(distanceMoved);
    ...
    /// 处理体力消耗逻辑
    /// </summary>
    private void HandleStaminaConsumption(float distanceMoved)
    {
    ...
    /// 处理随机事件触发判定
    /// </summary>
    private void HandleEventTrigger(float distanceMoved)
    {
        ...
        if (distanceSinceLastEventCheck >= eventCheckInterval)
        {
            ...
            {
                // 动态更新幸运值，防止变化未同步
                randomEventManager.SetLuckValue(stats.GetLuck());
    ...
    /// 处理随机事件触发结果
    /// </summary>
    private void OnRandomEventTriggered(RandomEventsDatabase.RandomEventData eventData)
    {
        if (eventData != null)
        {
            Debug.Log($"触发事件：{eventData.event_text}");
            ProcessEventEffects(eventData);
        }
        else
        {
            Debug.Log("未触发具体随机事件");
    ...
    /// 处理事件的实际奖励和惩罚逻辑
    /// </summary>
    private void ProcessEventEffects(RandomEventsDatabase.RandomEventData eventData)
    {
        // 处理奖励
